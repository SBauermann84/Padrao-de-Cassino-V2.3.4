...
import { 
  ...
  MinedRoulettePattern, 
  RoulettePatternCategory,
...
interface RoulettePatternMiningPanelProps {
  history: GameResult[];
  onApplyMinedStrategies?: (strategies: Strategy[]) => void;
  existingStrategies?: Strategy[];
}

export const RoulettePatternMiningPanel: React.FC<RoulettePatternMiningPanelProps> = ({
  history,
  onApplyMinedStrategies,
  existingStrategies = []
}) => {
  const [minedPatterns, setMinedPatterns] = useState<MinedRoulettePattern[]>([]);
  const [filterCategory, setFilterCategory] = useState<RoulettePatternCategory | 'all'>('all');
  ...
  useEffect(() => {
    if (history) {
      ...
      const timer = setTimeout(() => {
        const mined = mineRoulettePatterns(history);
  ...
  const togglePatternActive = (id: string) => {
    ...
    if (onApplyMinedStrategies) {
      const activeStrategies = convertMinedRoulettePatternsToStrategies(updated.filter(p => p.isActive));
    ...
    <div className="bg-[#111111] rounded-3xl border border-[#c6a34f]/20 p-5 md:p-6 space-y-6 shadow-2xl relative overflow-hidden">
      ...
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 border-b border-white/10 pb-5">
          ...
          <div className="flex items-center gap-2">
            ...
            <div>
              <h3 className="text-base md:text-lg font-black text-white uppercase tracking-wider flex items-center gap-2">
                Mineração Inteligente de Padrões Roleta
              </h3>
              <p className="text-xs text-zinc-400">
                O motor de inteligência analisa as rodadas da roleta em segundo plano, testa padrões em Apostas Externas, Dúzias, Colunas, Setores e Terminais, e ranqueia por assertividade.
              </p>
            </div>
          </div>
        </div>
        ...
        >
          ...
          <span>{isMining ? 'Analisando...' : 'Minar Padrões Roleta'}</span>
        </button>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        <div className="bg-white/[0.02] border border-white/10 rounded-2xl p-3.5 space-y-1">
          <p className="text-[10px] text-zinc-400 uppercase font-black tracking-wider">Padrões Extraídos</p>
          <p className="text-xl md:text-2xl font-black text-white font-mono">{minedPatterns.length}</p>
          <p className="text-[10px] text-zinc-500">Mesa e Setores</p>
        </div>
        ...
        <div className="bg-emerald-500/[0.03] border border-emerald-500/20 rounded-2xl p-3.5 space-y-1">
          ...
          <p className="text-[10px] text-emerald-500/70 truncate">
            {bestPattern ? bestPattern.categoryLabel : 'Aguardando dados'}
          </p>
        </div>
        ...
        <div className="flex items-center gap-1 overflow-x-auto custom-scrollbar pb-1 sm:pb-0">
          <button
            ...
            className={`px-3 py-1.5 rounded-xl text-[11px] font-black uppercase tracking-wider transition-all cursor-pointer whitespace-nowrap ${
              filterCategory === 'all' ? 'bg-[#c6a34f] text-black shadow' : 'text-zinc-400 hover:text-white hover:bg-white/5'
            }`}
          >
          ...
          <button
            ...
            className={`px-3 py-1.5 rounded-xl text-[11px] font-black uppercase tracking-wider transition-all cursor-pointer whitespace-nowrap ${
              filterCategory === 'external' ? 'bg-[#c6a34f] text-black shadow' : 'text-zinc-400 hover:text-white hover:bg-white/5'
            }`}
          >
            Apostas Externas
          </button>
          <button
            type="button"
            onClick={() => setFilterCategory('dozen_column')}
            className={`px-3 py-1.5 rounded-xl text-[11px] font-black uppercase tracking-wider transition-all cursor-pointer whitespace-nowrap ${
              filterCategory === 'dozen_column' ? 'bg-[#c6a34f] text-black shadow' : 'text-zinc-400 hover:text-white hover:bg-white/5'
            }`}
          >
            Dúzias & Colunas
          </button>
          <button
            type="button"
            onClick={() => setFilterCategory('sector')}
            className={`px-3 py-1.5 rounded-xl text-[11px] font-black uppercase tracking-wider transition-all cursor-pointer whitespace-nowrap ${
              filterCategory === 'sector' ? 'bg-[#c6a34f] text-black shadow' : 'text-zinc-400 hover:text-white hover:bg-white/5'
            }`}
          >
            Setores Cilindro
          </button>
          <button
            ...
            className={`px-3 py-1.5 rounded-xl text-[11px] font-black uppercase tracking-wider transition-all cursor-pointer whitespace-nowrap ${
              filterCategory === 'terminal' ? 'bg-[#c6a34f] text-black shadow' : 'text-zinc-400 hover:text-white hover:bg-white/5'
            }`}
          >
            Terminais
          </button>
      ...
      {filteredPatterns.length === 0 ? (
        <div className="text-center py-10 px-4 bg-white/[0.01] rounded-2xl border border-white/5 space-y-2">
          ...
          <p className="text-sm font-bold text-zinc-300">
            {history.length < 5 ? 'Lance pelo menos 5 rodadas de roleta para iniciar a mineração automática de padrões.' : 'Nenhum padrão encontrado com os filtros selecionados.'}
          </p>
        </div>
      ) : (
              ...
              >
                ...
                <div className="flex items-center justify-between pt-2 border-t border-white/5 text-[11px] text-zinc-300">
                  ...
                  <div className="flex items-center gap-1 font-bold">
                    ...
                    <span className="px-2 py-0.5 rounded text-[10px] font-black bg-[#c6a34f] text-black uppercase">
                      {pattern.predictedEntry}
                    </span>
                  </div>
                </div>
              </div>
...
export default RoulettePatternMiningPanel;
